...
export default function RecommendMenu() {
  ...
  return (
    <div className="h-[80vh] bg-red-400 px-5 pt-5 text-white md:px-14 lg:px-28 xl:px-44 2xl:px-72">
      <div className="flex h-full flex-col justify-around text-center">
        <div className="text-2xl">
          <p>
            오늘은 {label}요일, {rank}님을 위한 날.
          </p>
        </div>

        <div>
          <p className="mb-5 text-2xl">음식 종류 고르기</p>
          <div className="flex justify-center gap-3">
            {foodList.map((foodType, index) => (
              <div
                key={index}
                className={`cursor-pointer rounded-full p-3 ${selectedFood.includes(foodType) ? "bg-red-500 text-white" : "bg-white text-black"}`}
              >
                <input
                  type="checkbox"
                  value={foodType}
                  ...
                  className="hidden"
                />
                <label htmlFor={foodType} className="cursor-pointer">
                  {foodType}
                </label>
              </div>
            ))}
        ...
        <div className="text-2xl sm:text-3xl">
          <p>
            {rank}님은 오늘&nbsp;&#91;&nbsp;
            <span
              className={`text-2xl font-bold transition-opacity duration-700 sm:text-3xl ${isFading ? "opacity-100" : "opacity-0"}`}
            >
              {resultFood}
            </span>
            &nbsp;&#93;이 땡긴다.
        ...
        {navi && (
          <div>
            <Button
              onClick={() =>
                navigate(`/restaurant/${resultFood}/`, {
