...
export default function ReservationsManage() {
    ...
    const getTimeSlotUserReservations = async () => {
        ...
        try {
            const reservationPromises = paramList.map((params) => (
                api.get(`/owner/restaurants/${restaurantId}/timeslot-reservations`, { params, ...config })
            ));
    ...
    const handleUsedSeats = async (increase) => {
        ...
        try {
            const res = await api.patch(`/owner/restaurants/${restaurantId}/update-used-seats`, null, config);
            getMyRestaurant();
            getUserWaiting();
        } catch (err) {
            console.error(err);
    ...
    const cancelUserReservation = async (time, username) => {
        ...
        try {
            const res = await api.delete(`/owner/restaurants/${restaurantId}/timeslot-reservations/${username}`, config);
    ...
    const cancelUserWaiting = async (username) => {
        ...
        try {
            const res = await api.delete(`/owner/restaurants/${restaurantId}/queue-reservations/${username}`, config);
            alert(JSON.stringify(res.data.response));
            getUserWaiting();
        } catch (err) {
            console.error(err);
    ...
    const delayUserWaiting = async (username) => {
        ...
        try {
            const res = await api.patch(`/owner/restaurants/${restaurantId}/queue-reservations/postponed-guest-booking/${username}`, data, config);
    ...
    return (
        <div className="px-5 pt-5 md:px-14 lg:px-28 xl:px-44 2xl:px-72">
            {isLoading ?
                <Loading /> :
                <>
                    <p className="mb-3 text-lg">"{restaurantInfo.name}" 예약 / 테이블 관리</p>
                    <div className="flex flex-col gap-4">
                        ...
                        <div className=" bg-neutral-100 shadow-lg p-3">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="p-1 text-xl font-bold">{restaurantInfo.usedSeats} / {restaurantInfo.totalSeats}</span>
                        ...
                        <div className="border-b-[1px]">
                            ...
                            {Object.entries(reservations).filter(([timeKey, reservationList]) => reservationList.length > 0)
                                .map(([timeKey, reservationList]) => (
                                    ...
                                    >
                                        ...
                                        {reservationList.map((reservation, index) => (
                                            ...
                                            >
                                                <p key={index}>{index + 1}. {reservation.username}</p>
                                                <Button
                                                    className={tomatoBtn}
                                                    style={'text-sm'}
                                                    onClick={() => cancelUserReservation(timeKey, reservation.username)}
                                                >
                                                    예약 취소
                                                </Button>
                                            </div>
                        ...
                        <div>
                            ...
                            {userWaiting.sort((a, b) => a.booking - b.booking).map((user, index) => (
                                <div
                                    key={index}
