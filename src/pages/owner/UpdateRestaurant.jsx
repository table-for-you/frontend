...
export default function UpdateRestaurant() {
                ...
                if (decoded.role === "OWNER") {
                    ...
                    try {
                        ...
                        if (res.data.mainImage) {
                            setMainImage([{ file: res.data.mainImage, preview: res.data.mainImage }]);
                        }
    ...
    const updateMainImage = async () => {
        ...
        try {
            const res = await api.patch(`/owner/restaurants/${restaurantId}/main-image`, formData, config);
    ...
    const updateSubImages = async () => {
        ...
        const newImages = images.filter(image => {
            ...
            return image?.file && !originalImages.some(originalImage => originalImage === image.preview);
        });
        ...
        formData.append('deleteImageUrls', new Blob([JSON.stringify(deleteImageUrls)], { type: 'application/json' }));
        ...
        try {
            const res = await api.patch(`/owner/restaurants/${restaurantId}/sub-image`, formData, config);
        } catch (err) {
            console.error(err);
    ...
    const onSubmit = async () => {
        ...
        try {
            const res = await api.put(`/owner/restaurants/${restaurantId}`, data, config);
            if (res.status === 200) {
                setIsModalOpen(true);
            }
        } catch (err) {
